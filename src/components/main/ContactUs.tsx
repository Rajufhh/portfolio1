  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = data.error || 'Failed to send message. Opening email client...'
        setStatus(errorMessage)
        toast.error(errorMessage)
        
        // Open fallback email client if request failed
        const mailto = `mailto:rajubathini83@gmail.com?subject=${encodeURIComponent(
      ...
      setStatus('Message recorded! Opening your default email client...')
            ...
            >
              ...
              <div className="space-y-5 text-foreground">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium select-text">rajubathini83@gmail.com</span>
                  <button
                    title="Copy email address"
                    onClick={() => copyToClipboard('rajubathini83@gmail.com', 'Email')}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium select-text">+91-818-499-4937</span>
                  <button
                    title="Copy phone number"
                    onClick={() => copyToClipboard('+91-818-499-4937', 'Phone number')}
                    className="text-muted-foreground hover:text-primary transition"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium select-text">Hyderabad, Telangana, India</span>
                </div>
                <div className="pt-2 border-t border-primary/20">
                  <p className="text-xs text-muted-foreground uppercase font-semibold tracking-wider mb-2">Spoken Languages</p>
                  <div className="flex gap-2">
                    <span className="px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-semibold">English</span>
                    <span className="px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-semibold">Telugu</span>
                  </div>
                </div>
              </div>
            </motion.div>
